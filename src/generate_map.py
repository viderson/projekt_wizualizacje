...
main_group = folium.FeatureGroup(name='All Locations').add_to(map_fedropol)
groups = {
    'forest': folium.plugins.FeatureGroupSubGroup(main_group, 'las').add_to(map_fedropol),
    'stream': folium.plugins.FeatureGroupSubGroup(main_group, 'potok').add_to(map_fedropol),
    'mountain': folium.plugins.FeatureGroupSubGroup(main_group, 'góra').add_to(map_fedropol),
    'mound': folium.plugins.FeatureGroupSubGroup(main_group, 'kopiec').add_to(map_fedropol),
    'field': folium.plugins.FeatureGroupSubGroup(main_group, 'pole').add_to(map_fedropol),
    'meadow': folium.plugins.FeatureGroupSubGroup(main_group, 'łąki').add_to(map_fedropol),
    'pass': folium.plugins.FeatureGroupSubGroup(main_group, 'przełęcz').add_to(map_fedropol),
    'natural_region': folium.plugins.FeatureGroupSubGroup(main_group, 'region naturalny').add_to(map_fedropol),
    'historical_settlement': folium.plugins.FeatureGroupSubGroup(main_group, 'uroczysko-dawna miejscowość').add_to(map_fedropol),
}
...
for i, coord in enumerate(coordinates_list):
    ...
    folium.Marker(location=coord, popup=popup_text, icon=folium.CustomIcon(icon, icon_size=(31, 46))).add_to(group)
